...
namespace BrightistRenderer.Texts.Parsers
{
    internal class TextParser
    {
        ...
        public TextParser(ParserOptions options)
        {
            _options = options;
        }
        ...
        private ControlCodeCharacterData CreateControlCode(byte code, int[]? args = null)
        {
            switch (code)
            {
                ...
                case 10:
                    return args == null ?
                        new PlayerNameControlCodeCharacterData(Parse(_options.PlayerName), code) :
                        new PlayerNameControlCodeCharacterData(Parse(_options.PlayerName), code, args);
