# BrightisRenderer
A self-contained, .NET 8.0 app to render every text overlay in "Brightis" as they would show in-game.
...
The BrightistRenderer pairs best with another tool, [TranslationToSource](https://github.com/Brightis-Translation/TranslationToSource), which accepts translations desposited into the google sheets, and emits assembly source code ready to be injected into the game.
...
Unlike [TranslationToSource](https://github.com/Brightis-Translation/TranslationToSource), which requires a `sheetId` and `authorization pair`, here you have to provide them via a config.json (different from `overlay_config`) as such:
